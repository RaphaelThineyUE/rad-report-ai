...
export async function createTreatment(req: AuthRequest, res: Response): Promise<void> {
  ...
  logTreatmentAudit(req.userId, 'create_treatment', data.id, req.accessToken, String(req.ip ?? ''));
...
export async function updateTreatment(req: AuthRequest, res: Response): Promise<void> {
  ...
  logTreatmentAudit(req.userId, 'update_treatment', id, req.accessToken, String(req.ip ?? ''));
  res.json(data);
}
...
export async function deleteTreatment(req: AuthRequest, res: Response): Promise<void> {
  ...
  logTreatmentAudit(req.userId, 'delete_treatment', id, req.accessToken, String(req.ip ?? ''));
  res.status(204).send();
}
